...
describe("Pokemon", () => {
  ...
  it("테스트 환경입니다 >_< ", async () => {
    ...
    for (let i = 0; i < pokemonBiomes.length; i++) {
      const species = Species[pokemonBiomes[i][0]];
      const biomeEntries = pokemonBiomes[i][3];
    ...
    for (let i = 0; i < allSpecies.length; i++) {
      const x = allSpecies[i];

      const passive = starterPassiveAbilities[x.getRootSpeciesId()];
      const eggMoves: Moves[] = speciesEggMoves[x.getRootSpeciesId()].map((value, index, arr) => allMoves[value]);
      const moves = pokemonSpeciesLevelMoves[x.getRootSpeciesId()].map((value, index, arr) => [value[0], allMoves[value[1]]]);
      const biomes = biomeMap.get(Species[x.speciesId]);

      print2(x, pokemonId++, passive, eggMoves, moves, biomes);
      if (x.canChangeForm) {
        for (const form of x.forms) {
